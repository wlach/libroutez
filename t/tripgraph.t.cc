...
WVTEST_MAIN("getting_route_ids")
{
    ...
    WVPASSEQ(route_ids.size(), 1);
    WVPASS(*route_ids.begin() == 1);

    // an identical triphop, different service period, same route id
    g.add_triphop(500, 1000, 0, 1, 1, 1, "all2");
    route_ids = g.get_route_ids_for_stop(0, 0.0f);
    WVPASSEQ(route_ids.size(), 1);
    WVPASSEQ(*route_ids.begin(), 1);

    // an identical triphop, different service period, diff route id
    g.add_triphop(500, 1000, 0, 1, 2, 1, "all2");

    route_ids = g.get_route_ids_for_stop(0, 0.0f);
    WVPASSEQ(route_ids.size(), 2);
    WVPASS(*route_ids.begin() == 1 || *route_ids.begin() == 2);    WVPASS(*(++(route_ids.begin())) == 1 || *(++(route_ids.begin())) == 2);
    WVFAILEQ(*(route_ids.begin()), *(++(route_ids.begin())));
}
...
WVTEST_MAIN("find_triphops_for_stop")
{
    ...
    WVPASSEQ(ths.size(), 3);
    WVPASSEQ(ths[0].start_time, 500);
    WVPASSEQ(ths[1].start_time, 501);
    WVPASSEQ(ths[2].start_time, 501);
}
